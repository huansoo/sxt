    ...
    //仅做示例用，请自行修改
    //保存文件路径
    String filePath = "uploadimages";			//修改真实路径. edit by oiyc
    ...
    if(ServletFileUpload.isMultipartContent(request)){
    	    ...
    	    while(fii.hasNext()){
    	        FileItemStream fis = fii.next();
    	            ...
    	            if(!fis.isFormField() && fis.getName().length()>0){
    					...
    					ftype = matcher.group();
    					fileName = new Date().getTime()+"."+ftype;		//修改加了. edit by oiyc
        ...
        System.out.println("{'original':'"+originalName+"','url':'"+filePath.substring(filePath.lastIndexOf("/")+1,filePath.length())+"/"+fileName+"','title':'"+title+"','state':'"+state+"'}");
